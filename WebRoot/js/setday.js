...
if(calendar.DateMode >= pickMode["day"]){mvAry[mvAry.length] = 'display:none;';}// pickMode
																				// 精确到时日隐藏“时间”
...
if(calendar.DateMode >= pickMode["hour"]){mvAry[mvAry.length] = 'display:none;';}// pickMode
																					// 精确到小时时隐藏“分”
...
mvAry[mvAry.length] = '<span style="';
if(calendar.DateMode >= pickMode["minute"]){mvAry[mvAry.length] = 'display:none;';}// pickMode
																					// 精确到小时、分时隐藏“秒”
...
var now = new Date();
this.date = (dateObj.value.length > 0) ? new Date(dateObj.value.toDate(this.dateFormatStyle)) : now.format(this.dateFormatStyle).toDate(this.dateFormatStyle) ;// 2008-01-29
																																								// 寒羽枫添加
																																								// →
																																								// 若为空则根据dateFormatStyle初始化日期
if(this.panel.innerHTML==""||cal.dateFormatStyleOld != cal.dateFormatStyle)// 2008-01-29
...
this.dateControl.blurEvent = this.dateControl.onblur;}// 2007-09-14 保存主文本框的
														// onblur ，使其原本的事件不被覆盖
